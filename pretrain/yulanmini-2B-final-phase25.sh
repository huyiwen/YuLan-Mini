...
# ========== RESUME 只需要修改这里 ==========
last_stage_job_name=miniyulan-2B-final-phase24
STAGE=25
# ========================================
...
# warmup ratio
WARMUP_RATIO=0.0 # <-----第二个stage改这里

# weight decay
WEIGHT_DECAY=0.1

# deepspeed config path
DEEPSPEED_CONFIG_PATH='ds2_config_adamw_kd.json'
...
/usr/bin/pdsh -w $comma_hostnames bash torchrun_wrapper.sh $comma_hostnames $SLURM_JOB_NAME $SLURM_JOB_ID \
    ...
    --attention_bias True \
    --z_loss 0.0001 \
    --gradient_checkpointing_step 12 \
    --use_muparam_lr True \
